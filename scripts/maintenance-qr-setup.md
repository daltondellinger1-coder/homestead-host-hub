...
Outputs are written to `artifacts/maintenance-qr-codes/`:
- one print-ready branded PNG per active unit
- `contact-sheet.png` for quick review
- `homestead-hill-maintenance-qr-codes.zip` for sharing/printing
- `qr-target-urls.txt` for verification

If you run it before the real Tally form is published, the script creates DRAFT cards with `https://tally.so/r/REPLACE_ME` targets and a watermark so they cannot be mistaken for live QR codes.

### URLs generated by the script (replace `ABC123` with your form ID)
