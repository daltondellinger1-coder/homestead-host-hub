...
AS $$
  SELECT EXISTS (
    SELECT 1 FROM public.user_roles
     WHERE user_id = auth.uid() AND active = true AND role::text = ANY (_roles)
  );
$$;
...
WHERE NOT EXISTS (
  SELECT 1 FROM public.user_roles
   WHERE lower(email) = 'booking@homestead-hill.com'
     AND role::text = 'admin' AND active = true
);
...
ALTER TABLE public.units
  ADD COLUMN IF NOT EXISTS operational_status text NOT NULL DEFAULT 'vacant_ready'
    CHECK (operational_status IN ('occupied','vacant_ready','vacant_dirty','cleaning_scheduled','maintenance_needed','offline','under_renovation')),
...
CREATE TABLE IF NOT EXISTS public.reservations (
  ...
  booking_source text NOT NULL DEFAULT 'direct'
    CHECK (booking_source IN ('airbnb','furnished_finder','vrbo','booking_com','direct','long_term','lease','other')),
  status text NOT NULL DEFAULT 'confirmed'
    CHECK (status IN ('inquiry','tentative','confirmed','checked_in','checked_out','cancelled','no_show','extended')),
...
CREATE INDEX IF NOT EXISTS reservations_unit_dates_idx ON public.reservations (unit_id, check_in_date, check_out_date);
CREATE INDEX IF NOT EXISTS reservations_status_dates_idx ON public.reservations (status, check_in_date, check_out_date);
...
BEGIN
  IF NEW.status NOT IN ('confirmed','checked_in','extended')
     OR NEW.check_out_date IS NULL OR NEW.overlap_override THEN RETURN NEW; END IF;
  IF EXISTS (
    SELECT 1 FROM public.reservations r
     WHERE r.unit_id = NEW.unit_id AND r.id <> NEW.id
    ...
    RAISE EXCEPTION 'This unit already has an overlapping confirmed reservation.' USING ERRCODE = '23P01';
  END IF;
  RETURN NEW;
END;
$$;
...
ON public.reservations FOR EACH ROW EXECUTE FUNCTION public.prevent_reservation_overlap();
...
CREATE TABLE IF NOT EXISTS public.vendors (
  ...
  vendor_rank text NOT NULL DEFAULT 'primary' CHECK (vendor_rank IN ('primary','backup')),
...
CREATE UNIQUE INDEX IF NOT EXISTS vendors_active_trade_rank_unique
  ON public.vendors (lower(trade), vendor_rank) WHERE active = true;
...
CREATE TABLE IF NOT EXISTS public.cleaning_tasks (
  ...
  status text NOT NULL DEFAULT 'needs_scheduling'
    CHECK (status IN ('needs_scheduling','awaiting_confirmation','confirmed','in_progress','completed','readiness_verification_required','ready','cleaner_declined','overdue','cancelled')),
...
CREATE INDEX IF NOT EXISTS cleaning_tasks_action_idx ON public.cleaning_tasks (status, cleaning_deadline);
CREATE INDEX IF NOT EXISTS cleaning_tasks_cleaner_idx ON public.cleaning_tasks (assigned_cleaner_user_id, status);
...
CREATE TABLE IF NOT EXISTS public.operational_tasks (
  ...
  due_at timestamptz,
  priority text NOT NULL DEFAULT 'routine' CHECK (priority IN ('emergency','urgent','routine','low')),
...
CREATE INDEX IF NOT EXISTS operational_tasks_dashboard_idx ON public.operational_tasks (status, priority, due_at);
...
INSERT INTO public.approval_rules (category, threshold_amount, enabled, emergency_override_allowed)
VALUES
...
CREATE INDEX IF NOT EXISTS approval_requests_pending_idx ON public.approval_requests (status, created_at);
CREATE UNIQUE INDEX IF NOT EXISTS approval_requests_one_pending_per_record
  ON public.approval_requests (record_type, record_id, category) WHERE status = 'pending';
...
CREATE INDEX IF NOT EXISTS activity_log_record_idx ON public.activity_log (record_type, record_id, created_at DESC);
...
CREATE INDEX IF NOT EXISTS notifications_delivery_idx ON public.notifications (delivery_status, scheduled_at);
...
CREATE TABLE IF NOT EXISTS public.import_runs (
  ...
  confidence text NOT NULL DEFAULT 'medium' CHECK (confidence IN ('verified','high','medium','low')),
  status text NOT NULL DEFAULT 'preview' CHECK (status IN ('preview','confirmed','completed','failed','cancelled')),
...
BEGIN
  IF NEW.status = 'cancelled' THEN
    UPDATE public.cleaning_tasks SET status = 'cancelled', updated_at = now()
     WHERE departing_reservation_id = NEW.id AND status NOT IN ('completed','ready');
    RETURN NEW;
  END IF;
  IF NEW.check_out_date IS NULL
    ...
    RETURN NEW;
  END IF;
  SELECT * INTO v_next FROM public.reservations r
   WHERE r.unit_id = NEW.unit_id AND r.id <> NEW.id
     AND r.status IN ('confirmed','checked_in','extended')
     AND r.check_in_date >= NEW.check_out_date
   ORDER BY r.check_in_date, r.check_in_time LIMIT 1;
  ...
  INSERT INTO public.cleaning_tasks (
    unit_id, departing_reservation_id, next_reservation_id, checkout_at,
    next_check_in_at, cleaning_deadline, special_notes, updated_by
  ) VALUES (
    NEW.unit_id, NEW.id, v_next.id, v_checkout, v_next_checkin,
    coalesce(v_next_checkin, v_checkout + interval '24 hours'),
    NEW.cleaning_notes, auth.uid()
  )
  ON CONFLICT (departing_reservation_id) DO UPDATE SET
    ...
    status = CASE
      ...
      ELSE public.cleaning_tasks.status END
  RETURNING id INTO v_cleaning_task_id;

  INSERT INTO public.notifications (
    event_type, channel, template_key, delivery_status, sent_at,
    related_record_type, related_record_id, idempotency_key, payload
  ) VALUES (
    'cleaning_required','in_app','cleaning_required_v1','sent', now(),
    'cleaning_task', v_cleaning_task_id,
  ...
  INSERT INTO public.automation_events (event_type, record_id, idempotency_key, payload)
  VALUES ('cleaning.created', v_cleaning_task_id,
    'cleaning.created:' || v_cleaning_task_id::text,
    jsonb_build_object('cleaning_task_id', v_cleaning_task_id, 'unit_id', NEW.unit_id,
                       'departing_reservation_id', NEW.id, 'version', 1))
  ON CONFLICT (idempotency_key) DO NOTHING;

  IF NEW.status = 'checked_out' THEN
    UPDATE public.units SET operational_status = 'vacant_dirty', updated_by = auth.uid(), updated_at = now()
...
ON public.reservations FOR EACH ROW EXECUTE FUNCTION public.sync_cleaning_task_for_reservation();
...
BEGIN
  IF NEW.status = 'completed' AND OLD.status IS DISTINCT FROM NEW.status THEN
    ...
    IF nullif(trim(coalesce(NEW.supplies_needed, '')), '') IS NOT NULL THEN
      INSERT INTO public.operational_tasks (title, description, unit_id, cleaning_task_id, priority, status)
      VALUES ('Restock supplies after cleaning', NEW.supplies_needed, NEW.unit_id, NEW.id, 'urgent','new')
      ON CONFLICT (cleaning_task_id, title) WHERE cleaning_task_id IS NOT NULL AND status <> 'cancelled'
    ...
    IF nullif(trim(coalesce(NEW.damage_found, '')), '') IS NOT NULL
      ...
      INSERT INTO public.operational_tasks (title, description, unit_id, cleaning_task_id, priority, status)
      VALUES ('Review cleaner-reported damage or maintenance',
              concat_ws(E'\n', NEW.damage_found, NEW.maintenance_issue_found),
              NEW.unit_id, NEW.id, 'urgent','new')
      ON CONFLICT (cleaning_task_id, title) WHERE cleaning_task_id IS NOT NULL AND status <> 'cancelled'
      DO UPDATE SET description = EXCLUDED.description, updated_at = now();
    END IF;
  ELSIF NEW.status = 'ready' AND OLD.status IS DISTINCT FROM NEW.status THEN
    ...
    UPDATE public.units SET operational_status = CASE
      WHEN EXISTS (SELECT 1 FROM public.reservations r
                    WHERE r.unit_id = NEW.unit_id AND r.status = 'checked_in')
      THEN 'occupied' ELSE 'vacant_ready' END,
...
RETURNS trigger LANGUAGE plpgsql SECURITY DEFINER SET search_path = public
AS $$
DECLARE v_rule public.approval_rules; v_category text; v_approval_id uuid;
BEGIN
  v_category := CASE WHEN NEW.emergency THEN 'emergency_maintenance' ELSE 'maintenance' END;
  SELECT * INTO v_rule FROM public.approval_rules WHERE category = v_category AND enabled = true;
  IF v_rule.id IS NOT NULL AND v_rule.threshold_amount IS NOT NULL
    ...
    INSERT INTO public.approval_requests (category, record_type, record_id, amount, reason, status, requested_by)
    VALUES (v_category, 'maintenance_request', NEW.id, NEW.estimated_cost,
            'Estimated maintenance cost exceeds the configured owner-approval threshold.',
            'pending', auth.uid())
    ON CONFLICT DO NOTHING RETURNING id INTO v_approval_id;
  END IF;
  RETURN NEW;
END;
$$;
...
ON public.maintenance_requests FOR EACH ROW EXECUTE FUNCTION public.apply_maintenance_approval_threshold();
...
RETURNS trigger LANGUAGE plpgsql SECURITY DEFINER SET search_path = public
AS $$
DECLARE v_record_id text; v_action text; v_prior jsonb; v_new jsonb;
BEGIN
  v_record_id := coalesce(NEW.id, OLD.id)::text;
  v_action := lower(TG_TABLE_NAME) || '.' || lower(TG_OP);
  IF TG_OP = 'DELETE' THEN v_prior := jsonb_build_object('status', to_jsonb(OLD)->'status');
  ELSIF TG_OP = 'INSERT' THEN v_new := jsonb_build_object('status', to_jsonb(NEW)->'status');
  ELSE v_prior := jsonb_build_object('status', to_jsonb(OLD)->'status');
       v_new := jsonb_build_object('status', to_jsonb(NEW)->'status');
  END IF;
  INSERT INTO public.activity_log (actor_user_id, action, record_type, record_id, prior_value, new_value, source)
  VALUES (auth.uid(), v_action, TG_TABLE_NAME, v_record_id, v_prior, v_new,
...
DO $$ DECLARE v_table text; BEGIN
  FOREACH v_table IN ARRAY ARRAY['reservations','cleaning_tasks','operational_tasks','approval_requests','notifications']
  LOOP
    EXECUTE format('DROP TRIGGER IF EXISTS %I_activity_log ON public.%I', v_table, v_table);
    EXECUTE format('CREATE TRIGGER %I_activity_log AFTER INSERT OR UPDATE OR DELETE ON public.%I
       FOR EACH ROW EXECUTE FUNCTION public.log_operational_change()', v_table, v_table);
  END LOOP;
END $$;

DO $$ DECLARE v_table text; BEGIN
  FOREACH v_table IN ARRAY ARRAY['reservations','vendors','cleaning_tasks','operational_tasks',
  ...
  LOOP
    ...
    EXECUTE format('CREATE TRIGGER %I_updated_at BEFORE UPDATE ON public.%I
       FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column()', v_table, v_table);
  END LOOP;
END $$;

INSERT INTO public.reservations (
  external_reservation_id, guest_id, unit_id, booking_source, status,
  check_in_date, check_out_date, rate, payment_status, source_system, data_confidence
)
SELECT
  'legacy:' || g.id::text, g.id, g.unit_id,
  CASE g.source::text
  ...
  g.check_in, g.check_out, g.monthly_rate, 'unknown', 'legacy_host_hub', 'verified'
...
CREATE OR REPLACE FUNCTION public.create_reservation_with_guest(
  _unit_id uuid, _guest_name text, _guest_phone text, _guest_email text,
  _booking_source text, _check_in_date date, _check_out_date date,
  _check_in_time time DEFAULT '15:00', _check_out_time time DEFAULT '11:00',
  _special_notes text DEFAULT NULL
) RETURNS uuid LANGUAGE plpgsql SECURITY DEFINER SET search_path = public
AS $$
DECLARE v_guest_id uuid; v_reservation_id uuid; v_legacy_source public.booking_source;
BEGIN
  IF NOT public.has_any_role(ARRAY['admin','property_manager']) THEN RAISE EXCEPTION 'Not authorized.'; END IF;
  IF nullif(trim(_guest_name), '') IS NULL THEN RAISE EXCEPTION 'Guest name is required.'; END IF;
  ...
  v_legacy_source := CASE _booking_source
    ...
    ELSE 'other'::public.booking_source END;
  INSERT INTO public.guests (unit_id, name, phone, email, source, check_in, check_out, is_current, record_type, user_id)
  VALUES (_unit_id, trim(_guest_name), nullif(trim(_guest_phone), ''),
  ...
  ) VALUES (v_guest_id, _unit_id, _booking_source, 'confirmed',
...
REVOKE ALL ON FUNCTION public.create_reservation_with_guest(uuid, text, text, text, text, date, date, time, time, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.create_reservation_with_guest(uuid, text, text, text, text, date, date, time, time, text) TO authenticated;

CREATE OR REPLACE FUNCTION public.decide_approval_request(
  _approval_request_id uuid, _decision text, _decision_reason text DEFAULT NULL
) RETURNS boolean LANGUAGE plpgsql SECURITY DEFINER SET search_path = public
AS $$
DECLARE v_request public.approval_requests;
BEGIN
  IF NOT public.has_any_role(ARRAY['admin']) THEN RAISE EXCEPTION 'Owner access is required.'; END IF;
  ...
  UPDATE public.approval_requests
   ...
   WHERE id = _approval_request_id AND status = 'pending'
  RETURNING * INTO v_request;
  IF v_request.id IS NULL THEN RETURN false; END IF;
  IF v_request.record_type = 'maintenance_request' THEN
    UPDATE public.maintenance_requests SET approval_status = _decision WHERE id = v_request.record_id;
  END IF;
  RETURN true;
END;
$$;
